...
# 全局下载地址配置
DOCKER_COMPOSEV4_URL="https://raw.githubusercontent.com/Bibibiibi/flux-panel/refs/heads/main/docker-compose-v4.yml"
DOCKER_COMPOSEV6_URL="https://raw.githubusercontent.com/Bibibiibi/flux-panel/refs/heads/main/docker-compose-v6.yml"
GOST_SQL_URL="https://raw.githubusercontent.com/Bibibiibi/flux-panel/refs/heads/main/gost.sql"
...
install_panel() {
  ...
  docker run --rm -v "$(pwd)/ssl:/ssl" eclipse-temurin:21-jre \
    ...
    -dname "CN=flux-panel, OU=flux, O=flux, L=Local, ST=Local, C=CN" \
    2>/dev/null

  if [[ -f "ssl/keystore.p12" ]]; then
    echo "✅ SSL 证书生成成功 (有效期 10 年)"
  else
    echo "❌ SSL 证书生成失败，请检查 Docker 是否正常运行"
    exit 1
  fi
