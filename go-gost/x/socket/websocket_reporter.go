...
type WebSocketReporter struct {
	...
	tlsSkipVerify  bool              // 是否跳过TLS证书验证
}

// NewWebSocketReporter 创建一个新的WebSocket报告器
func NewWebSocketReporter(serverURL string, secret string, tlsSkipVerify bool) *WebSocketReporter {
...
func (w *WebSocketReporter) connect() error {
	...
	dialer := &websocket.Dialer{
		...
		TLSClientConfig: &cryptotls.Config{
			InsecureSkipVerify: w.tlsSkipVerify,
		},
	}
...
func StartWebSocketReporterWithConfig(addr string, secret string, http int, tls int, socks int, version string, tlsSkipVerify bool) *WebSocketReporter {

	// 构建初始 WebSocket URL（强制使用 wss:// 加密通讯）
	...
	fmt.Printf("🔗 WebSocket连接URL(TLS): %s\n", fullURL)

	reporter := NewWebSocketReporter(fullURL, secret, tlsSkipVerify)
