...
func main() {
	...
	wsReporter := socket.StartWebSocketReporterWithConfig(config.Addr, config.Secret, config.Http, config.Tls, config.Socks, "1.2.4", config.TlsSkipVerify)
	defer wsReporter.Stop()
	service.SetHTTPReportURL(config.Addr, config.Secret, config.TlsSkipVerify)
